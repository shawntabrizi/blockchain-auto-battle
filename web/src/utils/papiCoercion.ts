...
type MetadataVar =
  ...
  | { type: "enum"; value: Record<string, MetadataVar & { idx: number }>; innerDocs: Record<string, string[]> }
...
const encodeEnumValue = (value: any, def: MetadataVar) => {
  ...
  const variantName =
    typeof value === "string" ? value : value?.type ?? value?.tag;
  ...
  switch (unwrapped.type) {
    ...
    case "tuple":
      ...
      return value.map((item, index) =>
        encodeByVar(item, unwrapped.value[index]),
      );
    case "struct": {
...
const getCallArgVar = (
  ...
  if (!argDef) {
    throw new Error(
      `Call ${palletName}.${callName} missing arg ${argName}`,
    );
  }
