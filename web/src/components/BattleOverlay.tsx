  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-2 lg:p-4">
      <div className="bg-gray-900 rounded-xl p-3 lg:p-6 max-w-[98vw] lg:max-w-[95vw] w-full border border-gray-700 overflow-hidden flex flex-col max-h-[95vh] lg:max-h-[90vh] relative shadow-2xl">
        ...
        <div className="flex flex-col items-center gap-2 lg:gap-4 mt-2 lg:mt-4 flex-shrink-0 border-t border-gray-800 pt-2 lg:pt-4">
          <div className="h-10 lg:h-12 flex items-center justify-center">
            {battleFinished ? (
              <button
                onClick={onContinue}
                className={`btn ${isSandbox ? 'bg-gray-700 hover:bg-gray-600' : 'btn-primary'} text-sm lg:text-lg px-8 lg:px-16 py-2 animate-pulse shadow-[0_0_15px_rgba(234,179,8,0.3)]`}
              >
                {isSandbox ? 'Close' : 'Continue'}
              </button>
            ) : (
              <div className="text-gray-500 italic animate-pulse flex items-center gap-2 text-sm lg:text-base">
