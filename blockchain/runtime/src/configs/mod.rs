...
use sp_runtime::{traits::Hash as HashT, Perbill};
    ...
    > for ParentHashRandomness
{
    fn random(
        ...
        // 1. Retrieve the parent block hash (a robust proxy for current randomness for testing)
        let block_hash = frame_system::Pallet::<Runtime>::parent_hash();

        // 2. Mix the subject into the hash so different calls produce different results
        let mut subject_buf = subject.to_vec();
        subject_buf.extend_from_slice(block_hash.as_ref());

        // 3. Re-hash the result to get the final random seed
        // Using Blake2_256 or whichever hasher your runtime uses
        let random_seed = <Runtime as frame_system::Config>::Hashing::hash_of(&subject_buf);

        (random_seed, frame_system::Pallet::<Runtime>::block_number())
    }
}
